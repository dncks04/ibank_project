/*
 * k6 부하 테스트 — 동시 이체 처리량 & 잔액 정합성 검증
 *
 * 시나리오: 다수의 VU가 동일 출금계좌(A)에서 입금계좌(B)로 동시에 1원씩 이체한다.
 *   - A 계좌에 FOR UPDATE 락이 집중되어 락 경합(직렬화) 상황을 만든다.
 *   - 각 요청은 고유 멱등성 키를 사용한다.
 *   - 테스트 종료 후 (A 잔액 + B 잔액)이 초기 총액과 정확히 일치하는지 검증한다.
 *
 * 실행:
